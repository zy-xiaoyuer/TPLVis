       ...
       if the previous one is canceled (for cancelable events). */
         ...
         if not, update the grid to the current canvas state */
          ...
          function updateCanvasMask(maskCanvas) {
            ...
            for (var i1 = 0; i1 < imageData.data.length; i1 += 4) {
              var tone1 =
                imageData.data[i1] +
                imageData.data[i1 + 1] +
                imageData.data[i1 + 2];
              var alpha1 = imageData.data[i1 + 3];

              if (alpha1 < 128 || tone1 > threshold) {
                // Area not to draw
                newImageData.data[i1] = 0;
                newImageData.data[i1 + 1] = 0;
                newImageData.data[i1 + 2] = 0;
                newImageData.data[i1 + 3] = 0;
              } else {
                // Area to draw
                // The color must be same with backgroundColor
                newImageData.data[i1] = 255;
                newImageData.data[i1 + 1] = 255;
                newImageData.data[i1 + 2] = 255;
                newImageData.data[i1 + 3] = 255;
              }
            }
            ...
            function compatTextStyle(textStyle) {
              textStyle &&
                external_echarts_.util.each(compats, function (key) {
                  if (textStyle.hasOwnProperty.call(key)) {
                    textStyle[
